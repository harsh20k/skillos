...
source: "https://blog.bytebytego.com/p/a-guide-to-context-engineering-for?utm_source=substack&utm_medium=email&utm_campaign=email-restack-comment&r=6xqssj&triedRedirect=true"
author:
  - "[[ByteByteGo]]"
...
A 2025 Chroma study of 18 frontier models (GPT-4.1, Claude, Gemini) found that **every model performed worse as input grew** — not gradually, but with sudden cliff-drops. More context ≠ better output.

### Why LLMs Struggle with Long Context

- **Attention is not uniform** — models pay most attention to the beginning and end of input; the middle gets ignored ("lost in the middle" problem)
- **Computational cost scales quadratically** — doubling tokens ~quadruples computation
- **Context rot** — performance degrades unpredictably past a threshold, varying by model and task
- **Statelessness** — LLMs have no real memory; conversation history is just re-injected each call
- **Marketing vs. reality** — million-token windows pass needle-in-haystack tests but fail at real multi-document synthesis

### What is Context Engineering?

> Designing, assembling, and managing the entire information environment an LLM sees before it responds.

Prompt engineering = _how_ to phrase the instruction. Context engineering = _what_ to put in the window and how to structure it. As Karpathy put it: "the delicate art and science of filling the context window with just the right information for the next step."

**6 things competing for space in a context window:** system instructions, user input, conversation history, retrieved documents, tool definitions, tool outputs.

### 4 Core Strategies (from LangChain's categorisation)

|Strategy|Problem it solves|How|
|---|---|---|
|**Write**|Finite window + statelessness|Save to external storage (scratchpads, long-term memory like CLAUDE.md)|
|**Select**|More isn't better|RAG — retrieve only relevant chunks at query time|
|**Compress**|Context rot + cost|Summarise history (Claude Code auto-compacts at 95%), trim old messages, compress tool outputs|
|**Isolate**|Attention dilution|Multi-agent: each sub-agent gets a clean, focused context|

### Real-world Examples

- **Claude Code**: auto-compacts at 95% context capacity; uses CLAUDE.md as persistent memory
- **Anthropic multi-agent research system**: lead Opus 4 delegates to Sonnet 4 sub-agents → **90.2% improvement** over a single agent — purely from context management, not a stronger model
- **Cognition (Devin)**: trained a separate model just for summarisation at agent boundaries

### Open Tradeoffs

- **Single agent vs multi-agent** — Anthropic favours isolation; Cognition argues good compression in a single agent is more stable and cheaper
- **RAG precision** — imprecise retrieval adds distractors and backfires
- **Cost vs richness** — every extra token costs money; context engineering is partly an economics problem

### Bottom Line

_The model is only as good as the context it receives._ As models get more powerful, failures shift from intelligence failures to **context failures** — the model could have gotten it right, but didn't have what it needed (or had too much noise).

---

Directly applicable to your GroundSense work — especially the **isolate** strategy (your Bedrock multi-agent setup) and the **compress** strategy for managing long agentic workflows.
...
![|697x418](https://substackcdn.com/image/fetch/$s_!NrJV!,w_1456,c_limit,f_webp,q_auto:good,fl_progressive:steep/https%3A%2F%2Fsubstack-post-media.s3.amazonaws.com%2Fpublic%2Fimages%2Fad9e966a-1edf-4ddc-bf4b-14ab36e23b70_2910x1744.png)
...
![|697x419](https://substackcdn.com/image/fetch/$s_!GjNA!,w_1456,c_limit,f_webp,q_auto:good,fl_progressive:steep/https%3A%2F%2Fsubstack-post-media.s3.amazonaws.com%2Fpublic%2Fimages%2F9534a00d-5563-42ba-82e9-6c1a2aa1f04f_2526x1518.png)
...
![|697x419](https://substackcdn.com/image/fetch/$s_!Bdgk!,w_1456,c_limit,f_webp,q_auto:good,fl_progressive:steep/https%3A%2F%2Fsubstack-post-media.s3.amazonaws.com%2Fpublic%2Fimages%2Feadec10f-1ecc-4644-b35c-8627ddc69acb_2526x1518.png)
...
![|697x413](https://substackcdn.com/image/fetch/$s_!hrla!,w_1456,c_limit,f_webp,q_auto:good,fl_progressive:steep/https%3A%2F%2Fsubstack-post-media.s3.amazonaws.com%2Fpublic%2Fimages%2F4ce09eed-d88b-456b-a7c3-3ee5929a6067_4002x2368.png)
