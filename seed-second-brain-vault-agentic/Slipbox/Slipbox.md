...
```dataviewjs
// === SETTINGS ===
const ROOT = "Slipbox";     // "" for whole vault, or a folder like "Slipbox"
...
```dataviewjs
// === SETTINGS ===
const FOLDER = "Slipbox"; // Folder path to pick from

// Get all notes in the folder
let notes = dv.pages(`"${FOLDER}"`)
    .where(p => p.file.path.endsWith(".md"))
    .array();

// Bail if no notes found
if (notes.length === 0) {
    dv.paragraph(`No notes found in "${FOLDER}"`);
    return;
}
...
dv.header(2, `🎲 Random Note: ${randomNote.file.name}`);
let fileContent = await app.vault.read(app.vault.getAbstractFileByPath(randomNote.file.path));
